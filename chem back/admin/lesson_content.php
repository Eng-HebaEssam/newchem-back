                                        ...
                                        <div class="card-body ">
                                            ...
                                            <div class="table-responsive " id="printTable2">
                                                <table class="table" id="datatableid2">
                                                    ...
                                                    <tbody>
                                                    <?php 
                                                        $stmt = $con->prepare("SELECT parent FROM category where category_id = ?");
                                                        $stmt->execute(array($row['cat_id']));
                                                        $cat = $stmt->fetch();
                                                        $members = $con->prepare("SELECT members.*
                                                                                    FROM members 
                                                                                    Where members.groupid = ?
                                                                                    ORDER BY userid  desc");
                                                        $members->execute(array($cat['parent']));
                                                        $members = $members->fetchAll();
                                                        foreach($members as $member){
                                                            $lessons = $con->prepare("SELECT member_id FROM lesson_member 
                                                                                    Where lesson_id = ?
                                                                                    AND member_id = ?
                                                                                    ORDER BY lesson_member_id  desc");
                                                            $lessons->execute(array($lesson_id, $member['userid']));
                                                            $lessons = $lessons->fetchAll();
                                                            if(empty($lessons)){
                                                                echo '
                                                                    <tr>
